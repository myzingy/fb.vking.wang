...
END;
        ...
        if($ad_timespace=='today') {
            ...
            //asyn('apido/asyn.runRules', array('id' => $ad_id, 'type' => 'ad'),null,getDayTime($rule['exec_hour_minute'].':00',0,$ac['utc_seconds']));
            //其它Insights
            ...
            asyn('apido/asyn.flushAdsInsights',array('ad_id' => $ad_id,'ad_timespace'=>'lifetime','ac_id'=>$ac_id),null,
                getDayTime("02:01:00",1,$ac['utc_seconds']),0);

        }
        if($ad_timespace=='lifetime'){
            asyn('apido/asyn.getAssetForAd',array('ad_id' => $ad_id,'ac_id'=>$ac_id),
                null,getDayTime("20:00:00",0,$ac['utc_seconds']),0);
        }
        return $campaigns_data;
    }
